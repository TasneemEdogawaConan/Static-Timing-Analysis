...
{							     //name,    type,   outputname,   pin 
	...
	if (adj.size() != 0) // if it is the first 2/1 nodes
	{
		for (int j = 0; j < adj.size(); j++)
		{
			...
			{
				s2 = 1;
				if (t != "assigment" && (adj[j].type == "!" || adj[j].type == "")) //since sometimes inside the main function we pass portions of the nodes via another one. so we solve that by implementing this
					adj[j].type = t;
...
void StaticTiAnalysis::CreateGraphy()
{
	for (int y = 0; y < nodes.size(); y++)
	{
		if (nodes[y].type != "wire") // cause we need to insert the wire at the end , to correctly create the best level accuracy
		{
			...
			addEdge(nodes[y].name, nodes[y].type, nodes[y].output, "Outwire");
...
void StaticTiAnalysis::topologicalSortUtil(int s, int v, vector<cell> &adj, stack<string> &Stack, bool tillReg, bool& WE, bool&  reg_flag, double input_net_trans)
{
	...
	for (int j = 0; j < adj[v].edges.size(); j++)
	{
		...
		if ( tillReg == false && WE == true && p.size() != 0)
		{
			if (p.back().find(adj[v].node) != string::npos)// if it is an alternative root
			{
...
void StaticTiAnalysis::analyzeAndSort()
{
	cout << "Analyzing...\n";
	//calculate comb_delay
	for (int i = 0; i<paths.size(); i++)
	{
		for (int j = 0; j<paths[i].gate_delay.size(); j++)
		{
			paths[i].comb_delay += paths[i].gate_delay[i];
		}
	}

	//calculating arrival, required and slack
	for (int i = 0; i<paths.size(); i++)
	{
		paths[i].arrival = paths[i].comb_delay + paths[i].input_arrival;
		paths[i].required = clockperiod + paths[i].out_constraints;
		paths[i].slack = paths[i].arrival - paths[i].required;
	}

	cout << "Sorting...\n";
	//sort paths relative to their slacks
	for (int i = 0; i<paths.size(); i++)
	{
		for (int j = i; j<paths.size() - 1; j++)
		{
...
void  StaticTiAnalysis::outputTable()
{
	cout << "Clock cycle time = " << clockperiod << endl;
	for (int i = 0; i<paths.size(); i++)
	{
		cout << "Path number " << i << endl;
		cout << "-------------------------------------------------\n";
		for (int j = 0; j<paths[i].gate_name.size(); j++)
		{
			cout << "Gate : " << paths[i].gate_name[j] << " - Delay = " << paths[i].gate_delay[j] << endl;
		}
		cout << "-------------------------------------------------\n";
		cout << "Arrival time = " << paths[i].arrival << endl;
		cout << "Required time = " << paths[i].required << endl;
		cout << "Slack = " << paths[i].slack << endl;
		if (paths[i].slack<0)
			cout << "Violation exists\n";
		else
			cout << "No violation\n";
		cout << "-------------------------------------------------\n\n\n";
	}
